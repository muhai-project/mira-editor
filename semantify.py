...
def semantify_paper_batch(papers,api_key,max=None):
    ...
    for paper in tqdm(papers[0:max]):
        ...
        :hasEffectSize rdfs:domain :Hypothesis .
        :hasEffectSize rdfs:range :EffectSize .
        ...
        :moderatorEffectOnStatementStrength rdfs:range :EffectSize .
        ...
        7. a hypothesis describes the effect of an independent variable (such as social class or age) on a dependent variable (such as mortality). Optional variables are: a mediating variable (such as a country's living standards), which explains the process through which the independent and dependent variables are related, and a moderating variable which moderates the strength and direction of that relationship.

        8. for values of :hasRelation use mira:correlatesWith

        9. for effect sizes, choose from the following: :largeMediumNegativeES, :smallNegativeES, :nullFinding, :smallPositiveES, :largeMediumPositiveES)

        10. don't create your own identifiers but use blank nodes in case no IRI is available

        11. make sure to add an effect size for the relation between independent and dependent variable, but also to the moderator.
        ...
        @prefix : <https://w3id.org/mira/ontology/> .
        """
        try:
...
def process_graph(batch):
    ...
    old_prefixes = ['HumanPopulation','https://w3id.org/mira/ontology/hasStudy','Organization','GeographicRegion','GeographicalRegion',
                    'https://w3id.org/mira/ontology/representedBy','ObservationalStudy','https://w3id.org/mira/ontology/hasHypothesis',
                    'independentVariable','dependentVariable','http://semanticscience.org/resource/Hypothesis','moderatorVariable',
                    'mediatorVariable','https://w3id.org/mira/ontology/Sample']
    new_prefixes = ['SIO_001062','http://semanticscience.org/resource/SIO_000008','SIO_000012','SIO_000414','SIO_000414',
                    ...
                    'hasSubject','hasObject','https://w3id.org/mira/ontology/Explanation','hasModerator',
                    'hasMediator','http://semanticscience.org/resource/SIO_001050']
    ...
    construct {
        ...
        ?exp a mira:Explanation ;
            ...
            mira:moderatorEffectOnStatementStrength ?es ;
            mira:moderatorContext ?context ;
            mira:hasContext/sio:SIO_000205 ?sample .
        ?mod_var ?p ?o .
        ?mod_var rdfs:label ?label .
        BIND (IRI(CONCAT("http://example.org/", REPLACE(LCASE(STR(?label)), " ", "_"))) AS ?interaction)
    }
    """
